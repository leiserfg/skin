...
from os import listdir, getcwdu, walk, rename as os_rename
...
def rename(data, dest_path):
    for root, folders, files in walk(dest_path):
        for arr in [folders, files]:
            for i, f in enumerate(arr):
                if f.endswith('.tmpl'):
                    f = f[:-5]
                old_fullpath = join(root, f)
                tmpl = Template(f)
                new_name = tmpl.render(**data)
                if f != new_name:
                    pformat('rename', ' '.join([old_fullpath, 'to', new_name]),
                            color='green')
